...
def extracting_community_smells():
    ...
    for i, row in df.iterrows():
        ...
        if not os.path.exists(f'{project_name}.csv'):
            print(f"⚠️ Skipped: {project_name}.csv does not exists.")
            continue
        df_date = pd.read_csv(f'{project_name}.csv')
        output_csv = f'{project_name}_combined-result.csv'
        ...
        for idx in range(len(column)-1, -1, -1):
            ...
            text_file = f'{project_name}/release_{start_Date}/cs_results_{start_Date}'
