...
def calculateACCL(config, prCommentBatches, issueCommentBatches):
    for batchIdx, batch in enumerate(prCommentBatches):
        ...
        if len(issueCommentBatches) > 0:
            # issueCommentBatch = list([len(c) for c in issueCommentBatches])
            issueCommentBatch = []
            for id, row in enumerate(issueCommentBatches):
                l = len(issueCommentBatches[id])
                issueCommentBatch.append(l)
        else:
            issueCommentBatch = [0]
