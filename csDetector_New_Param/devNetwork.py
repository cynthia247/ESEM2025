...
from commitAnalysis_cynthia_endDate import commitAnalysis
import centralityAnalysis_Cynthia_EndDate as centrality
from tagAnalysis_Cynthia_EndDate import tagAnalysis
from devAnalysis import devAnalysis
from graphqlAnalysis.releaseAnalysis_Cynthia_EndDate import releaseAnalysis
from graphqlAnalysis.prAnalysis import prAnalysis
# from graphqlAnalysis.prAnalysis_Cynthia import prAnalysis
from graphqlAnalysis.issueAnalysis_Cynthia_EndDate import issueAnalysis
...
def devNetwork(argv):
        ...
        tagAnalysis(repo, delta, date, daysActive, config)

        coreDevs = centrality.centralityAnalysis(
            commits, delta, date, config)
        ...
        # print('------------------prCommentBatches length----------------: ', prCommentBatches)
        if all(len(batch) > 0 for batch in prCommentBatches):
            print('--------politeness working----------')
            politenessAnalysis(config, prCommentBatches, issueCommentBatches)
        result = {}
        

        # if len(prParticipantBatches) != 0 and len(issueParticipantBatches) != 0:
        #     politenessAnalysis(config, prCommentBatches, issueCommentBatches)
        # result = {}
        ...
        for batchIdx, batchDate in enumerate(date):
            
            if batchIdx > len(prParticipantBatches) and batchIdx > len(issueParticipantBatches):
                 break
            # get combined author lists
            if batchIdx < len(issueParticipantBatches) and batchIdx < len(prParticipantBatches):
                    combinedAuthorsInBatch = (
                        prParticipantBatches[batchIdx] +
                        issueParticipantBatches[batchIdx]
                    )
            elif batchIdx >= len(issueParticipantBatches) and batchIdx < len(prParticipantBatches):
                    combinedAuthorsInBatch+= prParticipantBatches[batchIdx]
            elif batchIdx < len(issueParticipantBatches) and batchIdx >= len(prParticipantBatches):
                    combinedAuthorsInBatch+= issueParticipantBatches[batchIdx]  
