...
def processBatch(batchIdx: np.int64, commits: List[Commit], config: Configuration):
    allRelatedAuthors = {}
...
def buildGraphQlNetwork(batchIdx: np.int64, batch: list, prefix: str, config: Configuration):
    allRelatedAuthors = {}
    authorItems = Counter({})
...
def prepareGraph(
    ...
    print("all related authors: ", len(allRelatedAuthors))
    # Cynthia
    if len(allRelatedAuthors) > 0:
